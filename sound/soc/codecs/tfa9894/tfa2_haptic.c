	...
	get_obj_cache_ptr(struct haptic_data *data, int idx)
{
	...
	if (idx >= (FW_XMEM_NR_OBJECTS1 + FW_XMEM_NR_OBJECTS2))
		...
		obj = (struct haptic_tone_object *)
			&data->object_table2_cache[idx - FW_XMEM_NR_OBJECTS1][0];
	else
		obj = (struct haptic_tone_object *)
	...
	struct haptic_data *data, int index)
{
	...
	pr_debug("starting obj[%d] of %d ms\n",
	...
	struct haptic_data *data, int index)
{
	...
	if ((obj->type == OBJECT_TONE) || (obj->type == OBJECT_SILENCE)) {
		...
		level = (data->amplitude * 0x7fffff + 50) / 100;
...
int tfa2_haptic_recalculate_wait(struct tfa2_device *tfa, int object)
{
	...
	if (object >= 0) {
		...
		tfa2_i2c_read_cf_mem32(tfa->i2c,
			...
			offsetof(struct haptic_tone_object,duration_cnt_max)/4,
			&duration,
			1,
			TFA2_CF_MEM_XMEM);
		/* duration is in sample count : 48k is 48 samples/ms */
...
static int tfa2_hap_recalc(struct tfa2_device *tfa, int object)
{
	...
	if ((80 < f0) && (f0 < 336)) {
		...
		if (tfa->hap_data.recalc_play_object) {
			...
			tfa2_i2c_read_cf_mem32(tfa->i2c,
				FW_XMEM_CMDOBJSEL0, &recalc_obj, 1, TFA2_CF_MEM_XMEM);
		} else
			recalc_obj = object;
...
int tfa2_hap_load_data(struct tfa2_device *tfa)
{
	...
	strncat(profile_name, "_data.hap", MAX_CONTROL_NAME);
